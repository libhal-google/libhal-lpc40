...
public:
  ...
  bool driver_initialize()
  {
    ...
    internal::pin(m_port.sda_port, m_port.sda_pin)
      .function(m_port.sda_function)
      .resistor(pin_resistor::none) // This probably shouldn't be none
      .open_drain(true);

    internal::pin(m_port.scl_port, m_port.scl_pin)
...
private:
  ...
  void configure_clock_rate()
  {
    ...
    m_port.reg->SCLL = static_cast<uint32_t>(kScll);
    m_port.reg->SCLH = static_cast<uint32_t>(kSclh);
  }
